...
if (len(sys.argv) > 2 and (
        sys.argv[2] == "C" or sys.argv[2] == "Python" or sys.argv[2] == "Py")):
...
def Parse_SmileBasic(sb_code, screen, usegfx, code_output):
    ...
    if (re.findall("^CCODE (.*)", sb_code, re.IGNORECASE) and codedone == False):
        ...
        codedone = True
        drawupdate = False
    if (re.findall("^GOTO \\@([A-Z0-9])", sb_code,
                   re.IGNORECASE) and codedone == False):
        glabel_exp = re.findall("^GOTO \\@([A-Z0-9])", sb_code, re.IGNORECASE)
        if (code_output == "C"):
            print "  goto " + glabel_exp[0] + ";"
        codedone = True
        drawupdate = False
    if (re.findall("^\\@([A-Z0-9])", sb_code,
                   re.IGNORECASE) and codedone == False):
    ...
    if (re.findall("^\'(.*)", sb_code, re.IGNORECASE) and codedone == False):
        ...
        codedone = True
        drawupdate = False
    if (re.findall("^GCLS ([0-9]+)", sb_code,
                   re.IGNORECASE) and codedone == False):
        ...
        if (code_output == "C"):
            ...
            # print "  boxRGBA("+draw_screen+", 0, 0, 255, 383,
            # "+Print_C_Color(SmileBasic_Graphic_Palette(int(gcls_exp[0])))+",
            # 255);";
        codedone = True
        drawupdate = True
    if (
        re.findall(
            "^GSPOIT\\s*\\(\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*\\)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gspoit_exp = re.findall(
            "^GSPOIT\\s*\\(\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*\\)",
            sb_code,
            re.IGNORECASE)
        ...
        codedone = True
        drawupdate = True
    if (re.findall("^GPAGE ([0-1])", sb_code,
                   re.IGNORECASE) and codedone == False):
        ...
        codedone = True
        drawupdate = False
    if (re.findall("^GCOLOR ([0-9]+)", sb_code,
                   re.IGNORECASE) and codedone == False):
    ...
    if (
        re.findall(
            "^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gpaint_exp = re.findall(
            "^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
        ...
        if (code_output == "C"):
            ...
            # print "  boxRGBA("+draw_screen+", 0, 0, 255, 383,
            # "+Print_C_Color(SmileBasic_Graphic_Palette(int(gpaint_exp[0])))+",
            # 255);";
        codedone = True
        drawupdate = True
    if (re.findall("^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
                   sb_code, re.IGNORECASE) and codedone == False):
        gpaint_exp = re.findall(
            "^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
        gpaint_exp = gpaint_exp[0]
        if (code_output == "Python" or code_output == "Py"):
        ...
        if (code_output == "C"):
            ...
            # print "  boxRGBA("+draw_screen+", 0, 0, 255, 383,
            # "+Print_C_Color(SmileBasic_Graphic_Palette(int(gpaint_exp[2])))+",
            # 255);";
        codedone = True
        drawupdate = True
    if (re.findall("^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)",
                   sb_code, re.IGNORECASE) and codedone == False):
        gpaint_exp = re.findall(
            "^GPAINT ([0-9]+)\\s*,\\s*([0-9]+)", sb_code, re.IGNORECASE)
        gpaint_exp = gpaint_exp[0]
        if (code_output == "Python" or code_output == "Py"):
        ...
        if (code_output == "C"):
            ...
            # print "  boxRGBA("+draw_screen+", 0, 0, 255, 383,
            # "+Print_C_Color(SmileBasic_Graphic_Palette(int(ptcolor)))+",
            # 255);";
        codedone = True
        drawupdate = True
    if (re.findall("^GSPOT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
                   sb_code, re.IGNORECASE) and codedone == False):
        gspot_exp = re.findall(
            "^GSPOT ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
        ...
        codedone = True
        drawupdate = True
    if (re.findall("^GSPOT ([0-9]+)\\s*,\\s*([0-9]+)",
                   sb_code, re.IGNORECASE) and codedone == False):
        gspot_exp = re.findall(
            "^GSPOT ([0-9]+)\\s*,\\s*([0-9]+)", sb_code, re.IGNORECASE)
        gspot_exp = gspot_exp[0]
        if (usegfx == False):
            if (code_output == "Python" or code_output == "Py"):
    ...
    if (
        re.findall(
            "^GLINE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gline_exp = re.findall(
            "^GLINE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
    ...
    if (
        re.findall(
            "^GLINE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gline_exp = re.findall(
            "^GLINE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
    ...
    if (
        re.findall(
            "^GBOX ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gbox_exp = re.findall(
            "^GBOX ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
    ...
    if (
        re.findall(
            "^GBOX ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gbox_exp = re.findall(
            "^GBOX ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
    ...
    if (
        re.findall(
            "^GFILL ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gfill_exp = re.findall(
            "^GFILL ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
    ...
    if (
        re.findall(
            "^GFILL ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gfill_exp = re.findall(
            "^GFILL ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
    ...
    if (
        re.findall(
            "^GCIRCLE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE) and codedone == False):
        gcircle_exp = re.findall(
            "^GCIRCLE ([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)",
            sb_code,
            re.IGNORECASE)
...
while (SB_i < SB_Num):
    if (SB_Exp[SB_i] != " "):
        Parse_SmileBasic(SB_Exp[SB_i].strip(),
                         "ds_screen", enable_gfxdraw, code_output)
    SB_i = SB_i + 1
if (code_output == "Python" or code_output == "Py"):
